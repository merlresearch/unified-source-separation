...
def generate_frame(bytes_array: bytes, frame_duration_ms: int, fs: int, process_tail: bool = False):
...
def compute_vad(
    ...
    vad_result = [
        ...
        for frame, timestamp in generate_frame(speech_bytes, frame_size, fs, process_tail=True)
    ]
    ...
    vad_mask = np.concatenate([np.full(frame_size_pts, is_speech) for _, _, is_speech in vad_result])[: len(speech)]
