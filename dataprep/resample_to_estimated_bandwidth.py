...
def resample_to_estimated_bandwidth(uid_path_bw, idx, max_files_per_dir, num_digits, outdir):
    ...
    try:
        ...
        print(f"{e}: cannot open audio file '{audio_path}'. Skipping it", flush=True)
        return
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the json/scp file containing audio paths and " "the corresponding bandwidth inforamtion",
    )
    parser.add_argument("--out_scpfile", type=str, required=True, help="Path to the output scp file")
    parser.add_argument(
        "--outdir",
        type=str,
        required=True,
    ...
    parser.add_argument("--chunksize", type=int, default=1, help="Chunksize for parallel jobs")
    parser.add_argument(
        "-m",
        "--max_files",
        type=int,
